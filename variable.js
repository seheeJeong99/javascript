...
//var 사용시 변수 선언 전에 값 할당 또는 출력 가능 => 쓰지말기!!!
// var hoisting : var 변수를 어디에 선언하든 가장 위로 끌어올림

//변수 const -> 변경 불가능한 변수(immutable). 읽기만 가능
//여러사람이 동시에 값 변경 불가능. 보안우수
const daysInWeek = 7;
const maxNumber = 5;

//변수 타입
//1. primitive : 원자성. number, string, boolean ... (immutable)
//2. object : box container (mutable)
...
// 2. object  --> 너무 커서 메모리에 한번에 올라가는게 불가능함. object를 가리키는 레퍼런스가 메모리에 저장됨
const sehee1 = {name: 'sehee', age: 25};
//const지만 바꿀수 있음 -> sehee1이 가리키는 포인터는(reference) 잠겨있지만 object안의 변수(name, age)는 변경이 가능함
sehee1.age = 24;    
console.log(sehee1);